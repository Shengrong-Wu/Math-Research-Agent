...
class Hyperparameters:
    ...
    N: int = 7  # max steps per roadmap (LLM may use fewer)
...
class AgentConfigs:
    ...
    falsifier: AgentProvider = field(default_factory=AgentProvider)
...
def load_config(path: Path | None = None) -> AppConfig:
    ...
    agents = AgentConfigs(
        ...
        falsifier=_load_agent_provider(agents_raw.get("falsifier", {})),
    )
    ...
    return AppConfig(
        ...
        agents=agents,
        lean=LeanConfig(
            toolchain=lean_raw.get("toolchain", "leanprover/lean4:v4.28.0"),
            mathlib=lean_raw.get("mathlib", True),
        ),
        skip_lean=hyper_raw.get("skip_lean", raw.get("skip_lean", False)),
        problem_id=raw.get("problem_id", ""),
        suite=raw.get("suite", ""),
        runs_dir=_PROJECT_ROOT / raw.get("runs_dir", "runs"),
    )
