"""Progressive context compression (4 layers).
...
Layer 4: Full context renewal -- start fresh, caller writes structured handoff (>95%)
"""
...
class ContextCompressor:
    ...
    When Layer 4 fires the compressor returns ``([], True)``.  The caller
    (Phase 1 runner) is responsible for:

    1. Building a :class:`~math_agent.documents.memo.HandoffPacket` from
       the agent's current working state.
    2. Saving it via ``memo.set_handoff(packet)``.
    3. Clearing the agent's context.

    This separation keeps the compressor stateless w.r.t. the proof state.
    """
    ...
    async def compress_if_needed(
        ...
        Returns ``(compressed_messages, was_reset)``.
        ...
        if pressure.level == "critical":
            ...
            # The caller must build a HandoffPacket and save it to MEMO
            # before clearing the agent's context.
            ...
            return [], True  # caller rebuilds from MEMO + handoff

        return result, False

    # ------------------------------------------------------------------
    # Layer 1 -- truncate long messages
    # ------------------------------------------------------------------
    ...
    async def _layer3_summarize(self, messages: list[LLMMessage]) -> list[LLMMessage]:
        ...
        summary_response = await self.client.generate(
            messages=[LLMMessage(
                role="user",
                content=(
                    "Summarize these proof conversation turns into a concise "
                    "summary preserving key results, failures, and proved "
                    f"propositions:\n\n{summary_text}"
                ),
            )],
            system=(
                "You are a mathematical proof assistant. Summarize concisely, "
                "preserving all proved results and failure reasons."
            ),
        )
